...
<% if(paging != null){ %>
		<div class="center" style="margin-top:50px; margin-left:45%;">
			<div class="pagination">
				<% if(paging.isPrev()){ %>
					<a href="/user/MylikeSaleShare?nowPage=<%=(paging.getPageBarStart()-1)%>">
						&laquo;
					</a>
				<%}%>
				<% for(int i = paging.getPageBarStart() ; i <= paging.getPageBarEnd() ; i++) {%>
					<a href="/user/MylikeSaleShare?nowPage=<%=i%>" 
					<%=paging.getNowPage() == i ? "class='active'" : ""%>>
						<%=i%>
					</a>
				<%}%>
				<% if(paging.isNext()){%>
					<a href="/user/MylikeSaleShare?nowPage=<%=(paging.getPageBarEnd()+1)%>">&raquo;</a>
				<%}%>
			</div>
		</div>
	<% } %>
